...
psi_amp = deg2rad(20);      % Sinusoidal heading amplitude
psi_period = 200;           % Sinusoidal heading period (s)
psi_ref = 0;                % Desired heading (initial)
...
for i=1:nTimeSteps
    ...
    % Desired heading (time-based sinusoid)
    omega_ref = 2 * pi / psi_period;
    psi_ref = psi_amp * sin(omega_ref * t(i));
...
figure(1)
shipSize = 1;  % Ship size for animation
set(gcf, 'Position', [1, 1, 0.4*scrSz(3), scrSz(4)]);
animateShip(x,y,shipSize,'-r',1);
