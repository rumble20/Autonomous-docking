...
fprintf('--- Step 0: container.m sanity check ---\n');
...
fprintf('--- Step 1: Module configuration ---\n');

% --- NMPC config --------------------------------------------------------
nmpc_cfg = struct();
nmpc_cfg.N  = 20;
nmpc_cfg.dt = 1.0;
nmpc_cfg.Q  = diag([2, 0.1, 0.1, 10, 10, 8, 0.01, 0.01, 0.1, 0.01]); % pos weights 10 (was 5)
nmpc_cfg.R  = diag([2, 0.001]);      % increased rudder penalty
...
nmpc_cfg.R_rate = diag([8, 0.001]); % strong rudder smoothing (was 0.5)
...
%% ========================================================================
function x_ref = buildRefTrajectory(x0, chi_d, U_d, N, dt_h, wp_pos, wp_idx)
    % Build reference by walking forward along the waypoint path.
    % Starts from the projected position of x0 onto the current segment,
    % so the MPC always sees a reference ON the path — not offset by XTE.
    %
    %   x0:      current state [10x1]
    %   chi_d:   desired heading from LOS [rad]  (used for psi reference)
    %   U_d:     desired speed [m/s]
    %   wp_pos:  Mx2 waypoints [x y; ...]
    %   wp_idx:  current active segment index
    
    n_wps = size(wp_pos, 1);
    
    % --- Project current position onto current segment ---
    i1 = wp_idx;
    i2 = min(wp_idx + 1, n_wps);
    seg_from = wp_pos(i1, :)';
    seg_to   = wp_pos(i2, :)';
    seg_vec  = seg_to - seg_from;
    seg_len  = norm(seg_vec);
    if seg_len > 1e-6
        t_proj = dot(x0(4:5) - seg_from, seg_vec) / (seg_len^2);
        t_proj = max(0, min(1, t_proj));
        path_pos = seg_from + t_proj * seg_vec;
    else
        path_pos = seg_from;
    end
    
    % --- Walk forward N steps along remaining path ---
    x_ref    = zeros(10, N+1);
    pos      = path_pos;
    cur_seg  = wp_idx;
    ...
    for k = 0:N
        ...
        % Advance pos by one step along path
        step_rem = U_d * dt_h;
        while step_rem > 1e-3
            next_wp = min(cur_seg + 1, n_wps);
            tgt     = wp_pos(next_wp, :)';
            d_to_wp = norm(tgt - pos);
            if d_to_wp < step_rem
                step_rem = step_rem - d_to_wp;
                pos = tgt;
                if cur_seg < n_wps - 1
                    cur_seg = cur_seg + 1;
                else
                    step_rem = 0;  % at final wp, stay
                end
            else
                dir = (tgt - pos) / max(d_to_wp, 1e-6);
                pos = pos + step_rem * dir;
                step_rem = 0;
            end
        end
    end
