...
config = struct();
config.N  = 20;           % 20 prediction steps
config.dt = 1.0;          % 1 second sample time → 20 s horizon (~140 m at 7 m/s)

% State weights:  [u   v    r    x    y    psi   p    phi  delta n   ]
config.Q = diag([  2   0.1  0.1  5    5    10    0.01 0.01 0.1  0.01]);
%  • psi weight = 10 : heading tracking is critical
%  • x,y weight = 5  : strong position tracking (was 0.01 — way too low)
%  • u weight = 2  : maintain desired speed

% Control weights (penalises DEVIATION from reference [0; nominal_RPM]):
config.R = diag([     1       0.001 ]);
%  • Rudder deviation from 0 is penalised (moderate)
%  • RPM deviation from nominal is almost free

config.use_obstacles = true;           % Enable constraints in NMPC
config.max_obs = 15;                   % Fixed obstacle slots in optimizer
config.use_map_obstacles = true;       % Integrate map polygons in NMPC
config.max_map_obs = 10;               % Closest polygons used each solve
...
for i = 1:length(t)
    ...
    if i == 1 || mod(i,20) == 0 || ~info.success
        fprintf('  [t=%5.1f]  pos=(%7.1f, %6.1f)  psi=%+5.1f deg  success=%d  rudder=%.3f deg  RPM=%.1f\n', ...
            t(i), x(4), x(5), rad2deg(x(6)), info.success, rad2deg(u_opt(1)), u_opt(2));
        if ~info.success && isfield(info, 'error') && i <= 5
            fprintf('    ERROR: %s\n', info.error);
        end
    end
...
for i = 1:length(t)
    ...
    if i == 1 || mod(i,20) == 0
        fprintf('  [t=%5.1f]  pos=(%7.1f, %6.1f)  psi=%+5.1f deg  success=%d\n  computation time: %.3f s\n', ...
            t(i), x(4), x(5), rad2deg(x(6)), info.success, info.solve_time);
    end
