...
d_to_active_end = norm(x(4:5) - wp_end_xy);
    ...
    if seg_end_idx < n_wps
        ...
        if heading_diff_deg >= route_follow_cfg.sharp_turn_deg
            % Request next-segment heading as target (but smooth it below)
            desired_goal_heading = chi_next_seg;
            desired_goal_enable = true;
            % Emphasize terminal/path heading to help negotiate sharp turns
